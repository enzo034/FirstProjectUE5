...
#define DRAW_SPHERE(Location) if (GetWorld()) DrawDebugSphere(GetWorld(), Location, 25.f, 24, FColor::Black, true);
#define DRAW_SPHERE_SingleFrame(Location) if (GetWorld()) DrawDebugSphere(GetWorld(), Location, 25.f, 24, FColor::Black, false, -1.f);
...
#define DRAW_CYLINDER(Location) if(GetWorld()) DrawDebugCylinder(GetWorld(), Location, Location + GetActorUpVector() * 50, 20.f, 12, FColor::Red, true);
